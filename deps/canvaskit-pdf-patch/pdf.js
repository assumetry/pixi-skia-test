// PDF export helpers — same pre-js scope as memory.js (preamble.js … postamble.js).
...
CanvasKit._extraInitializations.push(function() {
  ...
  CanvasKit.Document.prototype.beginPage = function(width, height, rect) {
    let rPtr = rect ? copyRectToWasm(rect) : 0;
  ...
  function initPDFMetadata(metadata) {
    ...
    // Default = 0 in SkPDF::Metadata::CompressionLevel; avoid closure renaming the enum.
    if (metadata['compressionLevel'] == null) {
      metadata['compressionLevel'] = 0;
    }
    metadata['_rootTag'] =
      metadata['_rootTag'] ||
      (metadata['rootTag'] ? initPDFTag(metadata['rootTag']) : null);
    return metadata;
  }
